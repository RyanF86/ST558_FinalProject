...
This document continues from the water potability dataset EDA with the goal of selecting a model that predicts potability from the other variables.  Both classification tree and random forest models will be considered.  The log-loss metric will be used to select the winning model.
...
```{r}
ClassTree_spec <- decision_tree(tree_depth = 30, # set tree depth at 30 
                                min_n = 2, # set minimum node size at 2  
                                cost_complexity = tune()) |> # tune cost complexity parameter
  set_engine("rpart") |>
  set_mode("classification")
```

Note: `tree_depth` and `min_n` are deliberately set to allow very large trees.  `cost_complexity` alone controls tree size.
...
Additionally, at each split only a random subset of the predictors is eligible for selection.  Without this restriction, the same strong predictors would tend to be selected for splitting in every tree.  Forcing variety can improve the efficacy of averaging all results.  The number of eligible predictors per split is a tuning parameter selected through cross-validation.
